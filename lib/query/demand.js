  ...
  if (andChunks.length > 1) {
    ...
    if (orChunks.length > 1) {
      ...
      orChunks.forEach(function (orChunk) {
        ...
        var orValue = value;
        // JOIN
        var dotChunks = orChunk.split(_constants.DOT);
        if (dotChunks.length > 1) {
          orKey = '' + _constants.JOIN + dotChunks[0];
          orValue = getQueryFromDemand('' + dotChunks.slice(1).join(_constants.DOT) + _constants.EQUAL + value);
        }
        // set
        or[orKey] = orValue;
      });
